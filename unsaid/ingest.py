...
def run_pipeline(
    ...
    units1 = segment_all_sections(sections1)
    units2 = segment_all_sections(sections2)

    logger.info("FY%d: %d units total", year1, len(units1))
    logger.info("FY%d: %d units total", year2, len(units2))

    if not units1:
        logger.error("No Year-1 units extracted. Cannot continue.")
        sys.exit(1)
    ...
    logger.info("STEP 5  Judging %d Year-1 units + %d NEW candidates via Claude Opus",
                len(units1), len(new_candidates))
    logger.info("=" * 60)

    changes = judge_all(units1, candidates_per_y1, new_candidates, year1, year2)
